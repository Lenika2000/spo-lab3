...
static int connection_routine(server_connection_data_t *self);
static void *connection_routine_voidptr(void *ptr) {
    connection_routine(ptr);
    return NULL;
}

int connection_routine(server_connection_data_t *self) {
    int retcode = 1;
    while (retcode > 0 && self->is_active &&
...
static int acceptor_routine(server_data_t *shared);
static void *acceptor_routine_voidptr(void *ptr) {
    acceptor_routine(ptr);
    return NULL;
}

int acceptor_routine(server_data_t *shared) {
    while (shared->is_running) {
        ...
        if (received_name == 1) {
            ...
            if (pthread_create(&data->thread, NULL,
                               connection_routine_voidptr, data) !=  0) {
...
int run_server(int argc, char **argv) {
    ...
    shared.is_running = 1;
    if (pthread_create(&shared.acceptor_thread, NULL,
                       acceptor_routine_voidptr, &shared) !=  0) {
